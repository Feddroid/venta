...
//*************************************** BASE DE DATOS WEBSQL
	...
	var urlP = "https://roinet.pe/tecnologiatextil/index.php/mobile_controller/";
...
//*********************************************** FIN BASE DE DATOS WEBSQL
	...
	$("body").on("click","#btn_guardar_frm", function(e){
		...
		if($('#accion_frm').val() == "editar"){
			...
			{
				editProspectoOn(id);
				//alert("ID EDITAR editProspectoOn SI");
				//$.mobile.changePage("#venta", {transition:"slideup"});
			}else{
				editProspectoOff(id);
				alert("ID EDITAR editProspectoOn NO");
				//$.mobile.changePage("#venta", {transition:"slideup"});
			}
			$.mobile.changePage("#venta", {transition:"slideup"});
		}else{

			alert("ID NO EDITAR "+id);
			/*if( $("#titpros").val() == ""){
		...
		$.ajax({
			...
			success : function(data) {
				if(data != 0){
					...
					$(".valor_emp_det_pros").html(data.presupuesto_pros+"<span> NUEVOS SOLES</span>");//PRESUESTO AQUI
